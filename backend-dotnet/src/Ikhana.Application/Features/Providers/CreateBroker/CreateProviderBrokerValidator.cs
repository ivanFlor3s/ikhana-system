...
public class CreateProviderBrokerValidator : AbstractValidator<CreateProviderBrokerCommand>
{
    private readonly IAppDbContext _context;

    public CreateProviderBrokerValidator(IAppDbContext context)
    {
        _context = context;
        ...
        RuleFor(x => x.ProviderId)
            .MustAsync(ProviderExists).WithMessage("The selected provider is invalid.");
    }
    ...
    private async Task<bool> ProviderExists(long providerId, CancellationToken cancellationToken)
    {
        return await _context.Providers
            .AnyAsync(p => p.Id == providerId && p.DeletedAt == null, cancellationToken);
    }
}
