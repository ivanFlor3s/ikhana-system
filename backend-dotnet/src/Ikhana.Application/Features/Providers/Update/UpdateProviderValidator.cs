...
public class UpdateProviderValidator : AbstractValidator<UpdateProviderCommand>
{
    ...
    public UpdateProviderValidator(IAppDbContext context)
    {
        ...
        RuleFor(x => x.Cuit)
            .MaximumLength(20)
            .MustAsync(BeUniqueCuitExceptSelf).WithMessage("The CUIT has already been taken.");
    ...
    private async Task<bool> BeUniqueCuitExceptSelf(UpdateProviderCommand command, string? cuit, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(cuit))
            return true;

        return !await _context.Providers
            .AnyAsync(p => p.Cuit == cuit && p.Id != command.Id && p.DeletedAt == null, cancellationToken);
    }
